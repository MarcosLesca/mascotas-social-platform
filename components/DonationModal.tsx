...
const DonationModal: React.FC<DonationModalProps> = ({ campaign, isOpen, onClose, variant = 'modal' }) => {
  ...
  const normalizedWhatsapp = normalizePhoneNumber(campaign.whatsappNumber || '');
  const hasWhatsapp = !!campaign.whatsappNumber;
  const hasEmail = !!campaign.contactEmail;

  const waMessage = `Hola! Vi la campaña "${campaign.title}" para ${campaign.petName} y me gustaría ayudar.`;
          ...
          <div className={`p-6 sm:p-8 ${!isFullscreen ? 'max-h-[50vh] overflow-y-auto' : ''}`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                {/* Descripción */}
                <div>
                  <h3 className="text-xl font-bold mb-3">Historia:</h3>
                  <p className="text-black leading-relaxed">{campaign.description}</p>
                </div>
                 ...
                 {/* Información Bancaria */}
                  ...
                  <div className="space-y-4">
                    {/* CBU */}
                    <div>
                      <label className="block text-sm font-bold text-black mb-2">CBU / CVU</label>
                      <div className="flex items-center gap-2">
                          ...
                          {campaign.cbu}
                           ...
                           <button
                              onClick={() => navigator.clipboard.writeText(campaign.cbu)}
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                          ...
                          <button
                              onClick={() => navigator.clipboard.writeText(campaign.alias)}
                               className="p-3 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors text-gray-600"
                              title="Copiar Alias"
                          >
                              <span className="material-symbols-outlined text-xl">content_copy</span>
                  ...
                  <div className="bg-sky-500/5 rounded-2xl p-6">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                           ...
                           <span className="font-medium">{campaign.responsibleName}</span>
                      </div>
                       {hasWhatsapp && (
                        <div className="flex items-center gap-3">
                          ...
                          {waHref ? (
                            ...
                            <span className="font-medium">{campaign.whatsappNumber}</span>
                          )}
                        </div>
                      )}
                      {hasEmail && (
                ...
                <div className="bg-white dark:bg-white/5 rounded-2xl p-6 border border-sky-500/10">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-white/5">
                      ...
                      <span className="text-sm font-bold text-black">{campaign.petName}</span>
                    </div>
                    <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-white/5">
                      <span className="text-sm text-black/70">Tipo de Ayuda:</span>
                      <span className="text-sm font-bold text-black">{typeLabel}</span>
                    </div>
                     <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-white/5">
                      <span className="text-sm text-black/70">Fecha Límite:</span>
                      <span className="text-sm font-bold text-urgent-red">{campaign.deadline}</span>
                    </div>
                    <div className="flex justify-between items-center py-2">
                      <span className="text-sm text-black/70">Responsable:</span>
                      <span className="text-sm font-bold text-black">{campaign.responsibleName}</span>
                    </div>
                  </div>
                </div>
              </div>
