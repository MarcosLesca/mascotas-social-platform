...
const Home: React.FC<HomeProps> = ({ onToast }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (lostRes.error) {
        onToast('No se pudieron cargar las ultimas mascotas perdidas.', 'error');
      } else {
  ...
  const mixedRecentCards = useMemo(() => {
    ...
    const maxLength = Math.max(latestLost.length, latestAdoption.length, latestDonations.length);

    for (let i = 0; i < maxLength; i += 1) {
      if (latestLost[i]) {
        cards.push({ kind: 'lost', id: `lost-${latestLost[i].id}`, pet: latestLost[i] });
      }
      if (latestDonations[i]) {
        cards.push({ kind: 'donation', id: `donation-${latestDonations[i].id}`, campaign: latestDonations[i] });
      }
      if (latestAdoption[i]) {
        cards.push({ kind: 'adoption', id: `adoption-${latestAdoption[i].id}`, pet: latestAdoption[i] });
      }
    }
      ...
      <div className="relative px-4 md:px-10 lg:px-20 py-12" style={{ marginTop: '200vh' }}>
        <div className="max-w-[1440px] mx-auto space-y-8">
          <section className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-accent-teal font-black">Home Feed</p>
              <h2 className="text-3xl md:text-5xl font-black leading-tight">Recientes de la comunidad</h2>
              <p className="text-accent-teal mt-2">Mosaico con publicaciones de perdidos, adopciones y donaciones.</p>
            </div>
          ...
          {loading && mixedRecentCards.length === 0 ? (
            <p className="text-accent-teal text-sm">Cargando publicaciones recientes...</p>
          ) : mixedRecentCards.length === 0 ? (
            <div className="rounded-3xl border border-accent-teal/10 bg-white dark:bg-white/5 p-8 text-center">
              <h3 className="text-xl font-black">Sin publicaciones recientes</h3>
              <p className="text-accent-teal text-sm mt-2">Todavia no hay datos para mostrar en el feed.</p>
            </div>
          ) : (
            <section className="columns-1 md:columns-2 xl:columns-3 gap-6 [column-fill:_balance]">
              {mixedRecentCards.map((item, index) => {
                const imageHeightClass =
                  index % 4 === 0 ? 'h-64' : index % 3 === 0 ? 'h-44' : index % 2 === 0 ? 'h-56' : 'h-52';
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="p-5 space-y-2">
                        <h3 className="text-lg font-black leading-tight">{campaign.title}</h3>
                        <p className="text-sm text-accent-teal line-clamp-3">{campaign.description}</p>
                        <div className="pt-2 border-t border-accent-teal/10 text-xs text-accent-teal">
                          <p className="font-bold">Meta: ${campaign.goal.toLocaleString('es-AR')}</p>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-5 space-y-2">
                      ...
                      <p className="text-sm text-accent-teal">{pet.breed} · {pet.location}</p>
                      {pet.description && <p className="text-sm text-accent-teal line-clamp-3">{pet.description}</p>}
                      <div className="pt-2 border-t border-accent-teal/10 text-xs text-accent-teal">
                        {isLost ? (
