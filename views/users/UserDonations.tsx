...
import {
  fetchUserDonationCampaignReports,
  deleteDonationCampaignReport,
...
export default function UserDonations() {
  ...
  const loadReports = useCallback(async () => {
    ...
    const { data, error: fetchError } = await fetchUserDonationCampaignReports(user.id);
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeleting(id);
    const { error: deleteError } = await deleteDonationCampaignReport(id);
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-4 text-slate-600 font-medium">
          No tienes campañas de donación registradas
        </p>
        <p className="text-slate-500 text-sm mt-1">
          Crea una desde la sección de donaciones
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-black text-[#203553]">Mis Campañas de Donación</h2>
            ...
            <div className="flex gap-4 p-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="text-sm text-slate-600">
                      {report.pet_name} • {getTypeLabel(report.type)}
                    </p>
                  </div>
                ...
                <div className="mt-2 flex flex-wrap gap-2 text-xs text-slate-500">
                  <span>💰 Meta: ${report.goal.toLocaleString("es-AR")}</span>
