...
export default function UserLostPets() {
  const { user } = useAuth();
  ...
  const loadReports = useCallback(async () => {
    ...
    const { data, error: fetchError } = await fetchUserLostPetReports(user.id);
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeleting(id);
    const { error: deleteError } = await deleteLostPetReport(id);
  ...
  if (reports.length === 0) {
    return (
      <div className="text-center py-12">
        <span className="material-symbols-outlined text-4xl text-slate-400">search</span>
        <p className="mt-4 text-slate-600 font-medium">
          No tienes mascotas perdidas registradas
        </p>
        <p className="text-slate-500 text-sm mt-1">
          Publica una desde la sección de mascotas perdidas
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-black text-[#203553]">Mis Mascotas Perdidas</h2>
